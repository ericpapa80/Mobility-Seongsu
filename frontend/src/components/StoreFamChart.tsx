...
export default function StoreFamChart({ stores, settings }: Props) {
  ...
  const data = useMemo(() => {
    return settings.categories.map(cat => {
      const values = FAM_KEYS.map(key => {
        let sum = 0;
        for (const s of filtered) {
          if (s.category_bg === cat) sum += s.fam[key] ?? 0;
        }
        return sum;
      });
      return { cat, values };
  ...
  useEffect(() => {
    ...
    const x0 = d3.scaleBand().domain(FAM_KEYS).range([0, iw]).padding(0.25);
