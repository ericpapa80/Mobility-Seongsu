...
입력:
  1) pipeline/ref/bus_stop_locations.csv  (위치·노선·정류소명)
  2) pipeline/bronze/bus__seoul__{YYYYMM}.csv  (시간대별 승하차)

결합 키: NODE_ID = stops_id (보조: ARS_ID = stops_ars_no)

출력: pipeline/silver/bus_stops_hourly.json
"""
...
LOCATION_CSV = REF_DIR / "bus_stop_locations.csv"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-r", "--ridership",
        type=Path,
        default=BRONZE_DIR / "bus__seoul__201511.csv",
        help="승하차 CSV 파일 경로 (bronze)",
    )
    parser.add_argument(
        "-o", "--output",
        type=Path,
        default=SILVER_DIR / "bus_stops_hourly.json",
