...
- 1단계(bus_stops.py) 또는 정적 노선목록(pipeline/ref/bus_routes_static.txt) 사용
- 각 노선에 대해 CardBusTimeNew 호출 → 성수동 관련 정류장만 필터(선택) 또는 전 노선 정류장 수집
- 출력: pipeline/bronze/seoul_bus_{YYYYMM}_성수동노선_정류장별_시간대별_승하차.csv

사용 예:
  python pipeline/extractors/bus_ridership.py -m 202601
  python pipeline/extractors/bus_ridership.py -m 202601 --routes pipeline/ref/bus_routes_static.txt
"""
...
def run(use_ym: str, routes_file: Path, filter_stops: set[str] | None, output_dir: Path) -> None:
    ...
    if not routes:
        default_static = REF_DIR / "bus_routes_static.txt"
        routes = load_route_list(default_static)
    if not routes:
        # 1단계 결과에서 로드
        json_file = REF_DIR / "bus_stops_routes.json"
        if json_file.exists():
            with open(json_file, encoding="utf-8") as f:
                data = json.load(f)
            routes = list(data.get("route_ids", []))[:20]
        else:
            txt_file = REF_DIR / "bus_routes.txt"
            routes = load_route_list(txt_file)

    if not routes:
        print("노선 목록 없음. bus_stops.py 실행 또는 pipeline/ref/bus_routes_static.txt 확인")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--routes", type=Path, default=None,
                        help="노선 목록 파일 (없으면 pipeline/ref/bus_routes.txt → bus_routes_static.txt)")
    parser.add_argument("--filter-stops", type=Path, default=None,
                        help="성수동 정류장만 필터 시 pipeline/ref/bus_stops_routes.json 경로")
    parser.add_argument("-o", "--output", type=Path, default=BRONZE_DIR)
    args = parser.parse_args()

    routes_file = args.routes or REF_DIR / "bus_routes.txt"
    filter_set = None
