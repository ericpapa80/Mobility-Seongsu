...
async def explore_seoul_systematically(
    ...
    for idx, (ne_lng, ne_lat, sw_lng, sw_lat) in enumerate(bboxes, 1):
        ...
        # API 요청 및 응답 대기 (bbox 이동 후 cell-token 수집을 위한 충분한 대기)
        await asyncio.sleep(2)
...
async def collect_cell_tokens(
    ...
    async with async_playwright() as p:
        ...
        auto_save_task = None
        try:
            auto_save_task = asyncio.create_task(_periodic_auto_save(interval_seconds=30))
            logger.info("💾 주기적 자동 저장 시작 (30초 간격)")
        except Exception as e:
            logger.debug(f"주기적 저장 태스크 시작 실패 (무시): {e}")
            ...
            if auto_save_task:
                try:
                    ...
                    try:
                        ...
                        pass
                    logger.info("💾 주기적 자동 저장 태스크 종료")
                except Exception as e:
                    logger.debug(f"주기적 저장 태스크 종료 중 오류 (무시): {e}")
...
def _emergency_save(signum=None, frame=None):
    """강제 종료 시 저장을 위한 핸들러."""
    if _save_context['save_called']:
        return  # 이미 저장했으면 중복 저장 방지
    
    _save_context['save_called'] = True
    
    if not _save_context['output_file']:
        return
    
    try:
        # 현재까지 수집된 토큰과 기존 토큰 병합
        all_tokens = _save_context['existing_tokens'].union(collected_tokens)
        
        logger.info("\n⚠️ 강제 종료 신호 수신. 저장 시도 중...")
        
        save_tokens(
            tokens=all_tokens,
            access_tokens=collected_access_tokens,
            output_file=_save_context['output_file'],
            existing_access_token=_save_context['existing_access_token']
        )
        
        logger.info(f"💾 긴급 저장 완료: {len(all_tokens)}개 cell-tokens")
    except Exception as e:
        logger.error(f"❌ 긴급 저장 실패: {e}")


def _atexit_save():
    """프로그램 종료 시 자동 저장 (atexit)."""
    _emergency_save()


async def _periodic_auto_save(interval_seconds: int = 30):
    """주기적으로 자동 저장하는 백그라운드 태스크.
    
    Args:
        interval_seconds: 저장 간격 (초, 기본값: 30)
    """
    last_saved_count = 0
    ...
    while True:
        try:
            ...
            # 저장 컨텍스트가 설정되어 있고, 새로운 토큰이 수집되었는지 확인
            if not _save_context['output_file']:
                continue
            
            current_count = len(collected_tokens)
            ...
            if current_count > last_saved_count:
                try:
                    ...
                    # 중복 저장 방지 플래그는 사용하지 않음 (주기적 저장이므로)
                    save_tokens(
                        tokens=all_tokens,
                        access_tokens=collected_access_tokens,
                        output_file=_save_context['output_file'],
                    ...
                    new_tokens = current_count - last_saved_count
                    logger.info(f"💾 주기적 자동 저장 완료: 총 {len(all_tokens)}개 (새로 수집: {new_tokens}개)")
                    last_saved_count = current_count
                except Exception as e:
                    logger.debug(f"주기적 저장 중 오류 (무시): {e}")
        except asyncio.CancelledError:
            # 태스크가 취소되면 종료
            break
        except Exception as e:
            logger.debug(f"주기적 저장 태스크 오류 (무시): {e}")
            await asyncio.sleep(interval_seconds)
...
def main():
    ...
    # signal 핸들러를 위한 컨텍스트 설정
    _save_context['output_file'] = output_file
    _save_context['save_called'] = False
    ...
    # signal 핸들러 컨텍스트 업데이트
    _save_context['existing_tokens'] = existing_tokens
    _save_context['existing_access_token'] = existing_access_token
    
    # signal 핸들러 등록 (Windows에서는 SIGINT만 지원)
    try:
        signal.signal(signal.SIGINT, _emergency_save)
        if hasattr(signal, 'SIGTERM'):
            signal.signal(signal.SIGTERM, _emergency_save)
    except (ValueError, OSError) as e:
        logger.debug(f"Signal 핸들러 등록 실패 (무시 가능): {e}")
    
    # atexit 핸들러 등록 (이중 안전장치)
    ...
    try:
        ...
        # 정상 완료 시 save_called 플래그 설정 (atexit에서 중복 저장 방지)
        _save_context['save_called'] = True
        
    except KeyboardInterrupt:
        logger.info("\n⚠️ 사용자에 의해 중단되었습니다.")
        # 중단 전까지 수집된 것 저장
        if not _save_context['save_called']:
